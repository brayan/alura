...
public class EnviadorDeLanceTest {
    ...
    @Test
    public void deveMostrarMensagemDeFalhaQuandoLanceForMenorQueUltimoLance() {
        doThrow(LanceMenorQueUltimoLanceException.class).when(leilao).propoe(any(Lance.class));
        enviadorDeLance.envia(leilao, new Lance(new Usuario("Else"), 100));
        verify(manager).mostraAvisoLanceMenorQueUltimoLance();
    }

    @Test
    public void deveMostrarMensagemDeFalhaQuandoUsuarioComCincoLancesDerNovoLance() {
        doThrow(UsuarioJaDeuCincoLancesException.class).when(leilao).propoe(any(Lance.class));
        enviadorDeLance.envia(leilao, new Lance(new Usuario("Brayan"), 200));
